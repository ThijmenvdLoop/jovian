...
def sum_superkingdoms(classified_file, mapped_reads_file):
    ...
    Output:
        ...
        top-level taxonomy group per sample, also taking into account reads
        that were not assigned a top-level group ("not classified")
    """
    clas_df = pd.read_csv(classified_file, delimiter="\t")
    clas_df = strip_object_columns(clas_df)
    clas_df = add_legacy_superkingdom(clas_df)
    ...
    # N.B. Take into account reads that were not assigned a top-level group
    ...
    superkingdom_sums = pd.DataFrame(
        clas_df.groupby(["Sample_name", LEGACY_TOP_LEVEL_COLUMN]).sum()[
            ["mapped_reads"]
        ]
    )

    # Check all samples and top-level groups to find out where there is no data
    samples_superkingdoms_dict = {}
    for i in superkingdom_sums.index:
        if i[0] not in samples_superkingdoms_dict.keys():
            # If the sample name is not yet in the dictionary, add its current group
            samples_superkingdoms_dict[i[0]] = [i[1]]
        else:
            # If the sample is there already, add the current group
    ...
    for key, value in samples_superkingdoms_dict.items():
        for superkingdom in TOP_LEVEL_TAXA + ["not classified"]:
    ...
    new_df = new_df.sort_values(by=["Sample_name", LEGACY_TOP_LEVEL_COLUMN])
    ...
    final_df = pd.DataFrame(
        new_df.pivot(
            index="Sample_name",
            columns=LEGACY_TOP_LEVEL_COLUMN,
            values="mapped_reads",
        )
    )
...
def validate_numbers(df, log=False):
    ...
    for i in range(len(df)):
        ...
        reads_sum = (
            ...
            + df.loc[i, "Human"]
            + sum(df.loc[i, taxon] for taxon in TOP_LEVEL_TAXA)
...
def main():
    ...
    nrs_df["Remaining"] = nrs_df["Total_reads"] - (
        nrs_df["Low-quality"]
        + nrs_df["Human"]
        + sum(nrs_df[taxon] for taxon in TOP_LEVEL_TAXA)
        + nrs_df["Unclassified"]
    )
